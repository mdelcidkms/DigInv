...
Module ModMain
    ...
    Private Sub RetrieveInvoices()
        ...
        Dim RSOrdUpd, RS, RSTot As Object
        ...
        RSOrdUpd = Db.CreateDynaset(SQL, &H0)
        If RSOrdUpd.RecordCount = 0 Then Exit Sub 'Nothing to run.
        Se.begintrans() 'Begin the whole transaction here!!!
        'PRODUCTION ********************************
        ...
        If RS.RecordCount > 0 Then
            ...
            If gs_Hoststring = gs_UserName Then
                DirToOutputData = "\\JMXS2INV01\MASFAC_ESC\DATOS\XXXXXX" 'MASTEREDI REINSTALLED, AFTER THAT PREVIOUS PATH NOT WRKNG, WILL USE FULL PATH.
            Else
            ...
            For i = 1 To RS.RecordCount
                ...
                If RS!totalfreig.Value < 0 Then
                    DiscExists = True 'V 3.3c
                    DiscPer = RS!FRGHTALLOW
                End If
                ...
                Email = vbNullString & RS!Email.Value
                ...
                TotInvAmt = (Val(RS!TotalSales.Value) + Val(RS!TotalTax.Value) + Val(RS!totalredem.Value) + Val(RS!totalfreig.Value) + Val(RS!totalmisc.Value))
                NumDpt = vbNullString & RS!DEPTCODE.Value
                ClaveFormaPago = RS!formapago.Value '  "99"   '10/03/18  '---- 10/07/21 no more hard-code read from customer master
                If RS!totalmisc.Value > 0 Then '
                    ...
                    Serie = "C"
                    Folio = gs_BranchN & RS!invhdrnum.Value 'GetDigInvNum("N")
                    FolioCP = RS!invhdrnum.Value
                    TotalSales = VB6.Format(TotInvAmt, "##0.00") 'NOTA DE CARGO IS REPORTED AS SALES
                    ...
                    If Trim(vbNullString & RS!Memo.Value) = "SUPERMERCADO" Then '-------------------02/21/19  SIR 1702 --
                        RELTIP = "02"
                        FEPos = InStr(vbNullString & RS!message, "FACTURA ") '
                        If FEPos > 0 Then
                            ''Debug.Print RS!message
                            RefInvoice = Trim(Mid(RS!message, FEPos + 8, 9)) 'GRAB 9 CHARS AFTER FACTURA
                ...
                Else
                    ...
                    DocName = "FACTURA"
                    Serie = "A"
                    Folio = gs_BranchN & RS!invhdrnum.Value 'GetDigInvNum("I")
                    TotalSales = VB6.Format(RS!TotalSales.Value, "##0.00") 'Total sales is in the colum total sales???? because it separates other charges!!!, SAT????
                    If Trim(vbNullString & RS!invmsg1.Value) <> vbNullString Then MSG123 = "1:  " & RS!invmsg1.Value 'added on 02/08/13
                    MSG123 = MSG123 & " " & RS!invmsg2.Value & " " & RS!invmsg3.Value
                    ''ClaveFormaPago = "99"   '10/03/18   'now depends if it is NC to Fresko, we use 15, otherwise 99
                End If
                RFC = StripRFC(vbNullString & RS!RFC.Value) 'RFC is now in TaxID column
                'MetodoPago = "PPD" & vbNewLine 'ALWAYS USE PAGO POSTERIOR FOR ALL CUSTOMERS. -07/17/20 NO MORE
                MetodoPago = RS!MetodoPago.Value & vbNewLine
                ...
                DueDate = GetTermMX(RS) 'DueDate NOT BEING USED, WE USE TERMSDATE
                IVATasa = Val(RS!SALSTXPCNT.Value)
                If ((vbNullString & RS!msgprintcode.Value) = "B" Or (vbNullString & RS!msgprintcode.Value) = "I") And Trim(vbNullString & RS!message.Value) <> vbNullString Then
                    InvMsg = "2:  " & RS!message.Value
                End If
                ...
                HDR = HDR & "SERFOL" & Space(25 - Len("SERFOL")) & Serie & vbNewLine
                HDR = HDR & "FECEXP  " & VB6.Format(Now, "yyyy-mm-ddThh:mm:ss") & vbNewLine & "NOAPRO  " & NoApro & vbNewLine & "AOAPRO  " & AoApro & vbNewLine
                'NEW ----------
                ...
                'Debug.Print HDR
                If vbNullString & RS!COUNTRY <> "MEX" Then
                ...
                HDR = HDR & "DIAPAG  " & (vbNullString & RS!ardays.Value) & vbNewLine & "PDPPAG  " & (vbNullString & RS!DISCPCNT.Value) & vbNewLine & "MDPPAG  " & vbNewLine
                HDR = HDR & "NUMEOC  " & RS!custordnum.Value & vbNewLine & "FECHOC  " & VB6.Format(RS!orderdate.Value, "yyyy-mm-dd") & vbNewLine & "FECCON  " & VB6.Format(RS!ShipDate.Value, "yyyy-mm-dd") & vbNewLine
                ...
                HDR = HDR & "NOTAS1  " & InvMsg & vbNewLine & "NOTAS2  " & vbNewLine & "NOTAS3  " & MontoLetra & vbNewLine & "AGENTE  " & (vbNullString & RS!salesmanid.Value) & vbNewLine
                HDR = HDR & "PEDIDO  " & gs_BranchN & "-" & RS!ordernum.Value & vbNewLine & "TRANSP  " & (vbNullString & RS!shipvia.Value) & vbNewLine
                ...
                HDR = HDR & "REFFAC  " & vbNewLine & "NUMCLI  " & gstrBranchAlpha & "-" & RS!customerid.Value & vbNewLine & "NUMSAP  " & RS!invhdrnum.Value & vbNewLine
                ...
                HDRCP = HDRCP & "VERSIO  4.0" & vbNewLine & "SERFOL  CP " & vbNewLine & "NUMFOL  " & RS!invhdrnum.Value & vbNewLine '----01/10/22
                HDRCP = HDRCP & "FECEXP  " & VB6.Format(Now, "yyyy-mm-ddThh:mm:ss") & vbNewLine
                ...
                HDR = HDR & "CVEREGREC   " & RS!JMX_REGIMEN_INV & vbNewLine 'CVEREGREC is the correct nemonic for the customer.
                '*******=========================================******
                ...
                If RFC = "TSO991022PB6" Then '08/10/17 ADDED FOR SORIANA ADDENDA TO INCLUDE FOLIO O NOTA DE ENTRADA---->>>>mod on 07/03/18 now to also include numero de cita
                    Select Case (vbNullString & RS!groupid) 'Select Case RS!customerid.Value   '--------------->>>>>>>>>>>>>>03/12/18
                        ...
                        Case Else
                            FEPos = InStrRev(vbNullString & RS!message, "FE") '08/23/17 to get Folio Nota de Entrada from message looking from end of msg.
                            If FEPos > 0 Then
                                'FE = Trim$(Mid$(RS!message, FEPos + 2, 7)) 'Read 7 only -->03/09/18 ADDED TRIM$, BECAUSE WHEN SPACE BETWEEN FE AND #, WAS ACCEPTING SPACE
                                FE = Trim(Mid(RS!message, FEPos + 2)) '05/04/18 GRAB EVERYTHING AFTE FE
                                ''Debug.Print RS!message
                                ...
                                End If
                                'Debug.Print RS!MESSAGE
                                CitaPos = InStrRev(UCase(vbNullString & RS!message), "CITA") '================>>>>>>>>>>>>>>>07/03/18
                                If CitaPos > 0 Then
                                    Cita = Trim(Mid(RS!message, CitaPos + 4, FEPos - CitaPos - 4)) '-->>>07/03/18 TO GET THE LENGHT OF CITA.
                ...
                HDR = HDR & "RFCEMI  " & JFCRFC & vbNewLine & "NOMEMI  " & JFCName & vbNewLine & "EANEMI  " & JFCGln & vbNewLine & "NUMEMI  " & RS!JFCVENDORNUM.Value & vbNewLine
                ...
                HDR = HDR & "RFCREC  " & RFC & vbNewLine & "NOMREC  " & RS!Name.Value & vbNewLine
                HDR = HDR & "CALREC  " & RS!address.Value & vbNewLine
                HDR = HDR & "COLREC  " & RS!address2.Value & vbNewLine & "LOCREC  " & RS!city.Value & vbNewLine
                HDR = HDR & "MUNREC  " & RS!address3.Value & vbNewLine & "ESTREC  " & RS!soldstateabbr.Value & vbNewLine & "PAIREC  " & RS!COUNTRY.Value & vbNewLine
                If vbNullString & RS!zipcode.Value = vbNullString Then 'VERSION 4 DO NOT LET ZIP CODE TO BE NULL BECAUSE ERROR REPORT IS NOT CLEAR
                    HDR = HDR & "CODREC  " & "99999" & vbNewLine 'IF USING 99999 WHEN NULL, THEN ERROR WILL SAY DOMICILIO (ADDRESS INCORRECT)
                Else
                    HDR = HDR & "CODREC  " & RS!zipcode.Value & vbNewLine
                End If
                HDR = HDR & "EANREC  " & RS!BillGLN.Value & vbNewLine & "TELREC  " & RS!phonenum.Value & vbNewLine & "MAIL    " & Email & vbNewLine

                '*******================CLIENTE, PERO USAMOS JFCRFC FOR TRASLADO =========================******
                HDRCP = HDRCP & "RFCREC  " & JFCRFC & vbNewLine '& "NOMREC  " & RS!Name.Value & vbNewLine '& "CALREC  " & "AV.AÑO DE JUAREZ NO. 160-B" & vbNewLine ' 01/19/23  per MasterEDI  same as JFC
                HDRCP = HDRCP & "NOMREC  " & JFCName & vbNewLine '& "CALREC  " & RS!address.Value & vbNewLine
                ...
                '************SHIP TO INFO ************
                HDR = HDR & "RFCENT  " & RFC & vbNewLine & "NOMENT  " & RS!shipname.Value & vbNewLine & "CALENT  " & RS!shipaddr.Value & vbNewLine
                HDR = HDR & "COLENT  " & RS!shipaddr2.Value & vbNewLine & "LOCENT  " & RS!shipcity.Value & vbNewLine & "ESTENT  " & RS!shipstateabbr.Value & vbNewLine
                HDR = HDR & "MUNENT  " & RS!shipaddr3.Value & vbNewLine & "ESTENT  " & RS!shipstate.Value & vbNewLine & "PAIENT  " & RS!shipcountry.Value & vbNewLine
                HDR = HDR & "CODENT  " & RS!shipzip.Value & vbNewLine & "EANENT  " & RS!ShipGLN.Value & vbNewLine
                '*******================OCTOBER 2021=========================******
                HDRCP = HDRCP & vbNewLine & "NOMENT  " & RS!shipname.Value & vbNewLine & "CALENT  " & RS!shipaddr.Value & vbNewLine
                If Trim(vbNullString & RS!shipzip.Value) = vbNullString Then
                    HDRCP = HDRCP & "CODENT  " & RS!zipcode.Value & vbNewLine
                Else
                    HDRCP = HDRCP & "CODENT  " & RS!shipzip.Value & vbNewLine
                End If
                ...
                HDRCP = HDRCP & "  COM_CPT_VERSIO 3.0" & vbNewLine '2.0 " & vbNewLine 12/28/23 carta porte version 3.0
                ...
                HDRCP = HDRCP & "   COM_CPT_TOTDIST " & RS!distance & vbNewLine '  "200 " & vbNewLine 'TotalDistRec Total distancia recorrida en Km
                ...
                HDRCP = HDRCP & "      COM_CPT_UBI_FECHA " & VB6.Format(Now, "yyyy-mm-ddT12:00:00") & vbNewLine 'FechaHoraSalida
                ...
                HDRCP = HDRCP & "      COM_CPT_DOM_LOC 09 " & vbNewLine 'RS!city.Value & vbNewLine
                HDRCP = HDRCP & "      COM_CPT_DOM_MUN " & "007" & vbNewLine '& RS!address3.Value & vbNewLine
                HDRCP = HDRCP & "      COM_CPT_DOM_EST  CMX" & vbNewLine 'DIF " & vbNewLine  ----01/11/23
                HDRCP = HDRCP & "       COM_CPT_DOM_PAI " & "MEX " & vbNewLine 'RS!COUNTRY.Value & vbNewLine
                ...
                HDRCP = HDRCP & "      COM_CPT_UBI_NOM " & RS!shipname.Value & vbNewLine
                HDRCP = HDRCP & "      COM_CPT_UBI_FECHA " & VB6.Format(DateAdd(Microsoft.VisualBasic.DateInterval.Hour, 6, Now), "yyyy-mm-ddT12:00:00") & vbNewLine
                HDRCP = HDRCP & "      COM_CPT_UBI_DISTREC " & RS!distance & vbNewLine 'DistanciaRecorrida
                HDRCP = HDRCP & "      COM_CPT_DOM_CAL " & RS!shipaddr.Value & vbNewLine '
                If vbNullString & RS!shipstateabbr.Value = vbNullString Then
                    HDRCP = HDRCP & "      COM_CPT_DOM_EST " & RS!soldstateabbr.Value & vbNewLine
                Else
                    HDRCP = HDRCP & "      COM_CPT_DOM_EST " & RS!shipstateabbr.Value & vbNewLine
                End If
                HDRCP = HDRCP & "      COM_CPT_DOM_PAI MEX " & vbNewLine
                If Trim(vbNullString & RS!shipzip.Value) = vbNullString Then
                    HDRCP = HDRCP & "      COM_CPT_DOM_CP " & RS!zipcode.Value & vbNewLine
                Else
                    HDRCP = HDRCP & "      COM_CPT_DOM_CP " & RS!shipzip.Value & vbNewLine
                End If
                ...
                Call GetDtl(i, RS!invhdrnum.Value) 'GET DETAIL INFORMATION
                Call GetCPVehiculo(i, RS!invhdrnum.Value) 'GET INFORMATION REGARDING TRANSPORTATION VEHICLE ---OCTOBER 2021
                If ("" & RS!FRGCHARGID.Value) = "Y" Then 'only if freight charge is at a header level, at detail only charges.
                    If Val(RS!totalfreig.Value) < 0 Then 'discount
                        DiscAmt = System.Math.Abs(Val(RS!totalfreig.Value)) 'ASUMING DISCOUNT AMT. ONLY COMES FROM FREIGHT (AFTER CHECKING DB)
                        TotDiscAmt = TotDiscAmt + DiscAmt '02/09/18
                        TotImp_Prt = TotImp_Prt + Val(RS!totalfreig.Value) '---------DiscAmt
                    ElseIf Val(RS!totalfreig.Value) > 0 Then  'Freight Charge
                        ExtraCharge = VB6.Format(Val(RS!totalfreig.Value), "##0.00")
                        ...
                        DTL = DTL & "VALUNI  " & ExtraCharge & vbNewLine & "IMPORT  " & ExtraCharge & vbNewLine & "IMPBRU_PRT  " & ExtraCharge & vbNewLine
                        DTL = DTL & "IMPIVAIEPS  " & ExtraCharge & vbNewLine
                ...
                TOTAL = TOTAL & "SUBTOT_PRT  " & VB6.Format(TotImp_Prt, "#,##0.00;-#,##0.00;""") & vbNewLine
                ...
                If Trim("" & RS!invopt_liqtaxseparate) <> vbNullString Then ' 07/21/23  added if to get the total ieps only if we need to separate something-  null means do not separate anything
                    ...
                    'SqlTot = SqlTot & "from invdtl d, branch_item b where d.itemcode = b.itemcode and invhdrnum = " & RS!ordernum & vbNewLine
                    ...
                    If DocName = "NOTA DE CARGO" Then
                        ...
                        SqlTot = SqlTot & "FROM INVDTL D, branch_item b ,remark_code R WHERE INVHDRNUM = " & RS!ordernum & vbNewLine
                    ...
                    If RSTot.RecordCount > 0 Then
                        ...
                        For x = 1 To RSTot.RecordCount '06/19/23
                            ...
                            '    If ArrIeps_Sep_Xml(1, (n * 2 - 1)) = Val(RSTot!tatiep.Value) Then '---------------
                            ...
                            '--------------------------------------------------------------------------------------
                            f = InStr("" & RS!invopt_liqtaxseparate, "C") '07/21/23
                            If f = 0 Then f = InStr("" & RS!invopt_liqtaxseparate, "Y")
                            If RSTot!tatiep = 8 And f > 0 Then '= "C" Then   'C is 8%  '07/20/23
                                ...
                                TOTAL = TOTAL & "TIPFACIEP" & t & Space(5) & RSTot!TIPFACIEP & vbNewLine
                                TOTAL = TOTAL & "TATIEP" & t & Space(5) & RSTot!tatiep & vbNewLine
                                TOTAL = TOTAL & "IEPTRA" & t & Space(5) & RSTot!IEPTRA & vbNewLine
                                '---
                                SeparateIEPSAmt = SeparateIEPSAmt + RSTot!IEPTRA '07/21/23
                                '---
                                'here------------if handle this on detail do not handle in this by total-- doesn't seem right, so commented out for now ?????????????????????
                                ' If RSTot!tatiep = 8 Then
                                'IEPSnoIVA = IEPSnoIVA + RSTot!IEPTRA  'IEPSnoIVA  add to amt not taxable for 0% IVA traslado, only 8% is that case
                                ' End If
                            End If '07/20/23
                            '---------------------------------(B) BEER 26.5--OR (Y) EVERYTHING--------------------'07/21/23
                            f = InStr("" & RS!invopt_liqtaxseparate, "B")
                            If f = 0 Then f = InStr("" & RS!invopt_liqtaxseparate, "Y")
                            If RSTot!tatiep = 26.5 And f > 0 And RSTot!liquorcode = 1 Then ' 02/02/24 added and rstot!liquorcode = 1 it could be 26.5%, but sake not beer
                                t = t + 1 '06/19/23 ..SIR 2536.. xml error out if we do not use secuential iepx ... because not all types have separation on xml now I needed to change. version 4 we started separating everything on xml, bur new request says go according to customer setup
                                TOTAL = TOTAL & "TIPFACIEP" & t & Space(5) & RSTot!TIPFACIEP & vbNewLine
                                TOTAL = TOTAL & "TATIEP" & t & Space(5) & RSTot!tatiep & vbNewLine
                                TOTAL = TOTAL & "IEPTRA" & t & Space(5) & RSTot!IEPTRA & vbNewLine
                            ...
                            f = InStr("" & RS!invopt_liqtaxseparate, "A")
                            If f = 0 Then f = InStr("" & RS!invopt_liqtaxseparate, "Y")
                            If RSTot!tatiep = 26.5 And f > 0 And RSTot!liquorcode > 1 Then '
                                ...
                                TOTAL = TOTAL & "IEPTRA" & t & Space(5) & RSTot!IEPTRA & vbNewLine
                                '---
                                SeparateIEPSAmt = SeparateIEPSAmt + RSTot!IEPTRA
                                '---
                            End If
                            '--------------
                            f = InStr("" & RS!invopt_liqtaxseparate, "A")
                            If f = 0 Then f = InStr("" & RS!invopt_liqtaxseparate, "Y")
                            If RSTot!tatiep = 30 And f > 0 Then '
                                t = t + 1 '06/19/23 ..SIR 2536.. xml error out if we do not use secuential iepx ... because not all types have separation on xml now I needed to change. version 4 we started separating everything on xml, bur new request says go according to customer setup
                                TOTAL = TOTAL & "TIPFACIEP" & t & Space(5) & RSTot!TIPFACIEP & vbNewLine
                                TOTAL = TOTAL & "TATIEP" & t & Space(5) & RSTot!tatiep & vbNewLine
                                TOTAL = TOTAL & "IEPTRA" & t & Space(5) & RSTot!IEPTRA & vbNewLine
                            ...
                            f = InStr("" & RS!invopt_liqtaxseparate, "A")
                            If f = 0 Then f = InStr("" & RS!invopt_liqtaxseparate, "Y")
                            If RSTot!tatiep = 53 And f > 0 Then 'Mid$("" & RS!invopt_liqtaxseparate, 1) = "A") Or (Mid$("" & RS!invopt_liqtaxseparate, 1) = "Y") Then
                                t = t + 1 '06/19/23 ..SIR 2536.. xml error out if we do not use secuential iepx ... because not all types have separation on xml now I needed to change. version 4 we started separating everything on xml, bur new request says go according to customer setup
                                TOTAL = TOTAL & "TIPFACIEP" & t & Space(5) & RSTot!TIPFACIEP & vbNewLine
                                TOTAL = TOTAL & "TATIEP" & t & Space(5) & RSTot!tatiep & vbNewLine
                                TOTAL = TOTAL & "IEPTRA" & t & Space(5) & RSTot!IEPTRA & vbNewLine
                    ...
                    f = InStr("" & RS!invopt_liqtaxseparate, "S")
                    If f = 0 Then f = InStr("" & RS!invopt_liqtaxseparate, "Y")
                    If f > 0 Then 'Mid("" & RS!invopt_liqtaxseparate, 1) = "S" Then '07/14/23 don't get total sugar beverage if is not S, which is separate sugar
                        ...
                        SqlTot = SqlTot & "invhdrnum = " & RS!ordernum & " and lineliqtax >0 and lineprice > 0 and b.liqtax2 > 0 group by 'Cuota' " 'liqtax2" 01/24/18 group by cuota instead of liqtax2.
                        RSTot = Db.CreateDynaset(SqlTot, &H4)
                        'Debug.Print SqlTot
                        SugarTaxAmt = 0 '06/20/23
                        If RSTot.RecordCount > 0 Then
                            For s = 1 To RSTot.RecordCount
                                ...
                                'IEPSnoIVA = IEPSnoIVA + RSTot!IEPTRA  'IEPSnoIVA to be added to amount not taxable for 0% IVA traslado
                        ...
                        RSTot.Close()
                    End If '07/14/23
                End If '                            07/21/23  end if of  RS!invopt_liqtaxseparate <> vbnullstring then
                '!!!!!!!!!!!!!!!--------------
                ...
                If IEPS_NO_SeparateXml And TotalTax_New > 0 And (VB6.Format(TotAmtNotTaxable + TotAmtToBeTaxed + TotalTax_New, "###.00")) <> VB6.Format(TotInvAmt, "###.00") Then
                ...
                End If
                RSOrdUpd.edit()
                If Not SorianaErr Then RSOrdUpd!exported.Value = "Y" '08/23/17 ADDED IF
                RSOrdUpd!moduser.Value = "DigInv3"
                RSOrdUpd!moddate.Value = VB6.Format(Now, "mm/dd/yyyy hh:mm:ss")
                RSOrdUpd.Update()
                RSOrdUpd.movenext()
                ...
                If Left(RS!dockid, 1) <> "F" Then HDRCP = vbNullString
                If Not SorianaErr Then
                    If RS!exported <> "C" Then CkFileExists(DirToOutputData, HDR & DTL & TOTAL, "INV-" & Folio & "-" & RS!RFC & "_" & VB6.Format(Now, "yyyymmdd_hh-mm") & ".txt")
                    If Trim(HDRCP) <> vbNullString Then CkFileExists(DirToOutputData, HDRCP & DTLCPT & DTLCP & COMCP & TOTCP, "CTP-" & RS!invhdrnum & " - " & VB6.Format(Now, "yyyy-mm-dd at hh-mm") & ".txt")
                Else '09/15/17 changed to stop sending e-mail when error
                    'SendEmail "SORIANA INVOICE FOUND: " & RS!ORDERNUM.Value & ", BUT NOT FOLIO DE ENTRADA FOUND " & vbNewLine & "THIS ORDER WON'T PROCESS."
                End If
                RS.movenext()
            ...
            Se.committrans()
        Else
            ErrMsgLog = New String("*", 50) & vbNewLine & VB6.Format(Now, "mm/dd/yy hh:mm") & " -> ERROR:  No records found for invoices to print, but the program was run." & vbNewLine & "Error on RetrieveInvoices Routine.  Prog:  DigInv3." & vbNewLine & "PROGRAM ENDED WITH OUT PROCESSING ANYTHING!!!!" & vbNewLine & "SQL: " & vbNewLine & SQL & vbNewLine
...
ErrHndlr:
        ...
        ErrMsgLog = New String("*", 50) & vbNewLine & VB6.Format(Now, "mm/dd/yy hh:mm") & " -> ERROR:  " & Err.Number & "-->" & Err.Description & "." & vbNewLine & "Error on RetrieveInvoices Routine.  Prog:  DigInv3." & vbNewLine & "Total Invoices:  " & TotInvs & vbNewLine & vbNewLine & "SQL: " & vbNewLine & SQL & vbNewLine
        CkFileExists(DirToOutputError, ErrMsgLog, "ERR-Dig-Inv3_3.log")
        SendEmail(ErrMsgLog)
        End
    End Sub
    ...
    Private Sub GetDtl(ByRef H As Short, ByRef InvNum As String)
        ...
        If RS.RecordCount > 0 Then
            ...
            For i = 1 To RS.RecordCount
                ...
                IEPSTasa = Val(vbNullString & RS!liqtax.Value) 'added vbnullstring 12/10/14
                LiqTax2 = Val(vbNullString & RS!LiqTax2.Value)
                ...
                If RFC = "NWM9709244W4" Then
                    UPC = ("" & RS!jancode.Value) 'added on 02/28/13 for WM use Jancode first, if not found, use UPC.
                    If Val(UPC) = 0 Then UPC = ("" & RS!unitupccode.Value)
                ...
                Else 'Other than WM check upc first, if null, then look for Jan code
                    UPC = ("" & RS!unitupccode.Value) '12/06/10
                    If Val(UPC) = 0 Then UPC = ("" & RS!jancode.Value)
                End If
                If Val(UPC) = 0 Then UPC = vbNullString 'if both upc and jan code are 0, then don't print all zeroes, leave it blank
                If RFC = "NWM9709244W4" Then UPC = VB6.Format(Val(UPC), "0000000000000") 'WM wants 13 digits field. ---WAL-MART
                ItemDesc = vbNullString & RS!linedesc.Value '2012 I need just desc for Soriana
                ItemCatalog = vbNullString & RS!satItem.Value '3.3 version
                UOMCatalog = vbNullString & RS!satuom.Value '3.3 version
                ...
                '--------------------------------------------
                d = d + 1
                Eaprice = RS!INVEAPRICE.Value
                CSPrice = RS!INVCSPRICE.Value
                If RS!eashipped.Value > 0 And RS!csshipped.Value > 0 Then
                    Units = "CE"
                ElseIf RS!eashipped > 0 Then
                    Units = "EA"
                ...
                End If
                '---------
                If RS!linetax.Value - RS!lineliqtax > 0 Then 'LineTax has combined tax (iva & ieps), to get iva, substract both taxes.
                    IvaTasaDtl = IVATasa
                    TotAmtToBeTaxed = Val(CStr(TotAmtToBeTaxed)) + Val(RS!lineprice.Value) + Val(RS!lineliqtax.Value) 'SUBTAI
                    TotalTax = CStr(Val(TotalTax) + (Val(RS!linetax.Value) - Val(RS!lineliqtax.Value))) 'calculate totaltax based on detail
                Else
                    IvaTasaDtl = 0
                    TotAmtNotTaxable = Val(CStr(TotAmtNotTaxable)) + Val(RS!lineprice.Value) 'SUBTSI
                End If
                SeparateLiqTaxFlag = IepsSeparate(RS!invhdrnum.Value, RS!linenum.Value)
                '--- 07/14/23 separate liquour or not should be same in pdf and xml, we do not do it differently now
                ...
                '-----
                TotAmtBeforeTaxes = TotAmtBeforeTaxes + RS!lineprice.Value '01/23/18
                ...
                Select Case Units
                    Case Is = "EA"
                        UnitPrice = VB6.Format(Eaprice, "##0.00")
                        Importe = RS!lineprice.Value
                        TotImp = TotImp + Val(Importe)
                        If Val(CStr(IEPSTasa)) > 0 Then IEPSAmt = RS!lineliqtax.Value
                        If LiqTax2 > 0 Then IEPSAmt = RS!lineliqtax.Value
                        ...
                        If (SeparateLiqTaxFlag = "N" And Val(CStr(IEPSTasa)) > 0) Or (SeparateLiqTaxFlag = "N" And LiqTax2 > 0) Then 'Price should add IEPS on inv to be sent on EXTRA FIELDS' 12/23/13 ----------
                            If IEPSTasa > 0 Then UnitPrice = VB6.Format(Eaprice + (Eaprice * (IEPSTasa / 100)), "##0.00###") '01/17/18 added one more zero because it doesn't pass SAT checking if round up here
                            'PENDING-----------------------07/14/23----------------------------------------------
                            'NEED TO REPEAT THIS SAME MOD TO THE OTHER SECTIONS (CA, CA & EA) -------------
                            Importe = VB6.Format(Val(RS!lineprice.Value) + Val(RS!lineliqtax.Value), "##0.00")
                            If LiqTax2 > 0 Then UnitPrice = CStr(Val(Importe) / RS!eashipped.Value) '07/14/23 use this instead of bellow -- was off some cents per eaprice so total was not matching....
                            'If LiqTax2 > 0 Then UnitPrice = Format(Eaprice + (LiqTax2 / RS!eapercs.Value), "##0.00###") ' '01/17/18 added one more zero because it doesn't pass SAT checking if round up herE
                            IEPSAmt_Prt = vbNullString
                        End If
                        If IvaTasaDtl > 0 Then
                            LineIVA = CStr(Val(RS!linetax.Value) - Val(RS!lineliqtax.Value))
                        ...
                        DTL = DTL & "CANTID  " & RS!eashipped.Value & vbNewLine & "CANTID_EA  " & RS!eashipped.Value & vbNewLine & "DESCRI  " & ItemDesc & vbNewLine
                        DTL = DTL & "CANPAQ  " & RS!eashipped.Value & vbNewLine
                        DTL = DTL & "CANEMP  " & RS!eashipped.Value & vbNewLine & "UNIDAD  " & Units & vbNewLine & "CVESKU  " & RS!itemcode.Value & vbNewLine
                        DTL = DTL & "ESTILV     " & RS!itemcode.Value & vbNewLine '10/31/17 per Master EDI to be able to print item code on pdf.
                        ...
                        DTL = DTL & "CODUPC  " & UPC & vbNewLine & "PIEPEM  " & "1" & vbNewLine & "PIEPEM2 " & RS!eapercs.Value & vbNewLine & "CODDUN  " & vbNewLine
                        '************************NOVEMBER  2021
                        'HERE!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        ...
                        DtlWeight = CDbl(VB6.Format((RS!eashipped / RS!eapercs) * RS!grosskg, "###0.00"))
                        If DtlWeight = 0 Then DtlWeight = 0.01
                        TotWeight = TotWeight + DtlWeight
                        DTLCP = DTLCP & "      COM_CPT_MER_PKG " & DtlWeight & vbNewLine 'Format((RS!eashipped / RS!eapercs) * RS!grosskg, "###0.00000") & vbNewLine & vbNewLine      'itemweight e  PesoEnKg
                        ...
                        DTLCP = DTLCP & "         COM_CPT_CMER_CANTID " & RS!eashipped.Value & vbNewLine
                        ...
                        If Eaprice = 0 Then 'free
                            ...
                            'TotAmtNotTaxable = Val(TotAmtNotTaxable) + (0.01 * RS!eashipped.Value)
                            'TotAmtBeforeTaxes = TotAmtBeforeTaxes + (0.01 * RS!eashipped.Value)
                            'END VERSION 4 MOD
                            DiscAmt = 0.01 * RS!eashipped.Value 'DO NOT Accumulate total discount DiscAmt + (0.01 * RS!EASHIPPED.Value)
                        ...
                        Else
                            ...
                            Else
                                ...
                                Else
                                    ...
                                    DTL = DTL & "IMPORTIPE " & VB6.Format(Eaprice * RS!eashipped.Value, "##0.00") & vbNewLine
                        ...
                        If IEPSTasa = 8 Then '8% ieps but no iva, detail should show 0% IVA
                            DTL = DTL & "TASIPE  0" & vbNewLine & "MONIPE 0 " & vbNewLine & "FCTTASIPE   0.000000" & vbNewLine
                            DTL = DTL & "IMPORTIPE " & VB6.Format((Eaprice * RS!eashipped.Value) + CDbl(IEPSAmt), "##0.00") & vbNewLine
                            DTL = DTL & "TIPIPETR Tasa" & vbNewLine '------------& "TASIEP 0" & vbNewLine
                        ...
                        DTL = DTL & "IMPIVAIEPS  " & RS!linetotal.Value & vbNewLine
                        DTL = DTL & "PBRUDE_IEPS  " & VB6.Format(UnitPrice, "##0.00") & vbNewLine '01/02/18
                        ...
                        If ((SeparateLiqTaxFlag = "N" And Val(CStr(IEPSTasa)) > 0) Or (SeparateLiqTaxFlag = "N" And LiqTax2 > 0)) And IEPS_NO_SeparateXml Then '-->>>04/09/19 ****SIR # 1794*** ------------->>>>>>>>>>>
                            ...
                            DTL = DTL & "PBRUDE  " & VB6.Format(UnitPrice, "##0.00") & vbNewLine
                            DTL = DTL & "VALUNI  " & VB6.Format(UnitPrice, "##0.00") & vbNewLine
                            DTL = DTL & "IMPBRU  " & VB6.Format(CDbl(UnitPrice) * RS!eashipped.Value, "##0.00") & vbNewLine
                            DTL = DTL & "IMPORT  " & VB6.Format(CDbl(UnitPrice) * RS!eashipped.Value, "##0.00") & vbNewLine
                            '--VERSION 4  ---  NOT SURE IF NEED TO USE OBJIMP = 03 ---- WILL CHECK THIS LATER------------
                        Else 'SIR 1794 bellow is same, what changes is above.
                            If Val(CStr(IEPSTasa)) > 0 Then DTL = DTL & "TASIEP  " & IEPSTasa & vbNewLine & "MONIEP  " & IEPSAmt & vbNewLine 'VERSION 4.0 ADDED IF ONLY IF >0
                            DTL = DTL & "PBRUDE  " & VB6.Format(Eaprice, "##0.00") & vbNewLine
                            DTL = DTL & "VALUNI  " & VB6.Format(Eaprice, "##0.00") & vbNewLine
                            DTL = DTL & "IMPBRU  " & VB6.Format(Eaprice * RS!eashipped.Value, "##0.00") & vbNewLine
                            DTL = DTL & "IMPORT  " & VB6.Format(Eaprice * RS!eashipped.Value, "##0.00") & vbNewLine
                        End If 'SIR 1794
                        '********END OF MOD*******************SIR 1794********************************************************
                        If LiqTax2 > 0 Then '01/23/18---------SUGARY DRINKS!!!!!!!!!!------------------
                            ...
                            If DiscAmt = 0 And SeparateLiqTaxFlag = "Y" Then ' don't do this bellow for free items 07/14/23 ADDED SEPARATELIQTAXFLAG
                                ...
                                DTL = DTL & "IMPORTIEP    " & VB6.Format(Val(IEPSAmt) / IEPSperLiter, "0.##") & vbNewLine 'VERSION 4 USE ONLY TWO DIGITS, OTHERWISE ERRORS OUT
                                ...
                                DTL = DTL & "MONIEP " & Val(IEPSAmt) & vbNewLine
                                ...
                                If Val(CStr(IvaTasaDtl)) = 0 Then '01/26/23  the modificatin above, should actually be this one  ivatasadtl not lineiva.....
                                    DTL = DTL & "TASIPE 0 " & vbNewLine & "MONIPE 0" & vbNewLine & "FCTTASIPE 0.000000" & vbNewLine
                                    DTL = DTL & "TIPIPETR Tasa " & vbNewLine & "IMPORTIPE " & ((Eaprice * RS!eashipped.Value)) + CDbl(IEPSAmt) & vbNewLine
                        ...
                        If DiscExists Then
                            ...
                            DTL = DTL & "MDECON    " & (Eaprice * RS!eashipped.Value) * (DiscPer / 100) & vbNewLine
                        Else
                    ...
                    Case Is = "CA"
                        UnitPrice = VB6.Format(CSPrice, "##0.00")
                        Importe = RS!lineprice.Value
                        TotImp = TotImp + Val(Importe) 'Importe sin ieps
                        If Val(CStr(IEPSTasa)) > 0 Then IEPSAmt = RS!lineliqtax.Value 'SAT IEPSAmt
                        If LiqTax2 > 0 Then IEPSAmt = RS!lineliqtax.Value
                        If (SeparateLiqTaxFlag = "Y" And Val(CStr(IEPSTasa)) > 0) Or (SeparateLiqTaxFlag = "Y" And LiqTax2 > 0) Then
                            IEPSAmt_Prt = IEPSAmt 'Ieps printing
                            TotIEPS_Prt = CStr(Val(TotIEPS_Prt) + Val(IEPSAmt))
                        ...
                        If (SeparateLiqTaxFlag = "N" And Val(CStr(IEPSTasa)) > 0) Or (SeparateLiqTaxFlag = "N" And LiqTax2 > 0) Then 'Price should add IEPS on inv
                            If IEPSTasa > 0 Then UnitPrice = VB6.Format(CSPrice + (CSPrice * (IEPSTasa / 100)), "##0.00000")
                            If LiqTax2 > 0 Then UnitPrice = VB6.Format(CSPrice + LiqTax2, "##0.00000")
                            Importe = VB6.Format(Val(RS!lineprice.Value) + Val(RS!lineliqtax.Value), "##0.00") 'Importe con IEPS
                            IEPSAmt_Prt = vbNullString
                        End If
                        If Val(Importe) = 0 And TotInvAmt <> 0 Then 'when eaqty and csqty = 0, but detail exists, probably XTRA CHARGE!!!
                            Importe = VB6.Format(RS!lineprice.Value, "##0.00") 'ONLY WHEN EAQTY AND CSQTY = 0 WE WILL HAVE THIS SITUATION
                            UnitPrice = "0.00"
                        End If
                        If IvaTasaDtl > 0 Then
                            LineIVA = CStr(Val(RS!linetax.Value) - Val(RS!lineliqtax.Value))
                        ...
                        DTL = DTL & "D" & vbNewLine
                        DTL = DTL & "CANTID  " & RS!csshipped.Value & vbNewLine & "CANTID_CA  " & RS!csshipped.Value & vbNewLine & "DESCRI  " & ItemDesc & vbNewLine
                        DTL = DTL & "CANPAQ  " & RS!csshipped.Value & vbNewLine
                        DTL = DTL & "CANEMP  " & RS!csshipped.Value & vbNewLine & "UNIDAD  " & Units & vbNewLine & "CVESKU  " & RS!itemcode.Value & vbNewLine
                        DTL = DTL & "ESTILV     " & RS!itemcode.Value & vbNewLine '10/31/17 per Master EDI to be able to print item code on pdf.
                        DTL = DTL & "CVEPRODSERV     " & ItemCatalog & vbNewLine '10/31/17 New SAT item code
                        DTL = DTL & "CVEUNIDAD     " & UOMCatalog & vbNewLine '11/15/17 New for 3.3 version
                        DTL = DTL & "CODUPC  " & UPC & vbNewLine & "PIEPEM  " & RS!eapercs.Value & vbNewLine & "PIEPEM2 " & RS!eapercs.Value & vbNewLine & "CODDUN  " & vbNewLine
                        '*****************OCTOBER 2021*****************************
                        DTLCP = DTLCP & "COM_CPT_INIMER " & vbNewLine & vbNewLine 'Inicio de mercancia
                        DTLCP = DTLCP & "   COM_CPT_MER_BIENTRA " & ItemCatalog & vbNewLine 'BienesTransp (clave de producto)
                        DTLCP = DTLCP & "   COM_CPT_MER_DESCRI " & ItemDesc & vbNewLine
                        DTLCP = DTLCP & "   COM_CPT_MER_CANTID 1" & vbNewLine
                        DTLCP = DTLCP & "   COM_CPT_MER_CVUNID  " & UOMCatalog & vbNewLine '
                        DtlWeight = CDbl(VB6.Format(RS!csshipped * RS!grosskg, "###0.00"))
                        If DtlWeight = 0 Then DtlWeight = 0.01
                        TotWeight = TotWeight + DtlWeight
                        ...
                        DTLCP = DTLCP & "   COM_CPT_INICANTRAN " & vbNewLine 'Inicio canidad trasladada
                        DTLCP = DTLCP & "   COM_CPT_CMER_CANTID " & RS!csshipped.Value & vbNewLine
                        DTLCP = DTLCP & "      COM_CPT_CMER_IDORI OR000001" & vbNewLine
                        DTLCP = DTLCP & "     COM_CPT_CMER_IDDES DE000001" & vbNewLine
                        DTLCP = DTLCP & "   COM_CPT_FINCANTRAN" & vbNewLine
                        DTLCP = DTLCP & "COM_CPT_FINMER" & vbNewLine & vbNewLine
                        ...
                        If CSPrice = 0 Then 'free
                            ...
                            'TotAmtNotTaxable = Val(TotAmtNotTaxable) + (0.01 * RS!csshipped.Value)
                            'TotAmtBeforeTaxes = TotAmtBeforeTaxes + (0.01 * RS!csshipped.Value)
                            'END VERSION 4 MOD
                            DiscAmt = 0.01 * RS!csshipped.Value
                            IEPSTasa = 0 'In case free item has ieps do not report tasa because it won't have any ieps amount
                            LiqTax2 = 0 '01/25/24  In case free item, do not report any tasa, no ieps amt should be reported
                            TotDiscAmt = TotDiscAmt + DiscAmt
                        End If
                        ...
                        Else
                            ...
                            Else 'ivatasadtl = 0
                                ...
                                If DiscExists = False Then
                                    ...
                                    DTL = DTL & "IMPORTIPE " & VB6.Format(CSPrice * RS!csshipped.Value, "##0.00") & vbNewLine
                                    DTL = DTL & "TIPIPETR Tasa" & vbNewLine & "TASIEP 0" & vbNewLine
                                End If
                            End If
                        End If
                        If IEPSTasa = 8 Then '8% ieps but no iva, detail should show 0% IVA
                            DTL = DTL & "TASIPE  0" & vbNewLine & "MONIPE 0 " & vbNewLine & "FCTTASIPE   0.000000" & vbNewLine
                            DTL = DTL & "IMPORTIPE " & VB6.Format((CSPrice * RS!csshipped.Value) + CDbl(IEPSAmt), "##0.00") & vbNewLine
                            DTL = DTL & "TIPIPETR Tasa" & vbNewLine '-----------------------& "TASIEP 0" & vbNewLine
                            TotAmtNotTaxable = TotAmtNotTaxable + CDbl(IEPSAmt)
                        ...
                        If ((SeparateLiqTaxFlag = "N" And Val(CStr(IEPSTasa)) > 0) Or (SeparateLiqTaxFlag = "N" And LiqTax2 > 0)) And IEPS_NO_SeparateXml Then '-->>>04/09/19 ****SIR # 1794*** ------------->>>>>>>>>>>
                            ...
                            DTL = DTL & "PBRUDE  " & VB6.Format(UnitPrice, "##0.00") & vbNewLine
                            DTL = DTL & "VALUNI  " & VB6.Format(UnitPrice, "##0.00") & vbNewLine
                            DTL = DTL & "IMPBRU  " & VB6.Format(CDbl(UnitPrice) * RS!csshipped.Value, "##0.00") & vbNewLine
                            DTL = DTL & "IMPORT  " & VB6.Format(CDbl(UnitPrice) * RS!csshipped.Value, "##0.00") & vbNewLine
                        Else
                            If Val(CStr(IEPSTasa)) > 0 Then DTL = DTL & "TASIEP  " & IEPSTasa & vbNewLine & "MONIEP  " & IEPSAmt & vbNewLine 'VERSION 4 ADDED IF ONLY WHEN NON ZERO
                            DTL = DTL & "PBRUDE  " & VB6.Format(CSPrice, "##0.00") & vbNewLine
                            DTL = DTL & "IMPBRU  " & VB6.Format(CSPrice * RS!csshipped.Value, "##0.00") & vbNewLine
                            DTL = DTL & "VALUNI  " & VB6.Format(CSPrice, "##0.00") & vbNewLine
                            DTL = DTL & "IMPORT  " & VB6.Format(CSPrice * RS!csshipped.Value, "##0.00") & vbNewLine
                        End If
                        '---------
                        ...
                        If LiqTax2 > 0 Then '01/23/18---------SUGARY DRINKS!!!!!!!!!!  1.17 per liter ------------------
                            ...
                            If DiscAmt = 0 And SeparateLiqTaxFlag = "Y" Then '  02/09/18 don't do this bellow for free items'  02/09/18 don't do this bellow for free items AND don't do this if no separate 07/14/23 added
                                ...
                                DTL = DTL & "IMPORTIEP    " & VB6.Format(Val(IEPSAmt) / IEPSperLiter, "0.##") & vbNewLine '05/31/22 VERSION 4, USE ONLY TWO DECIMALS, OTHERWISE ERRORS OUT
                                '*************----------------******************
                                ...
                                If Val(CStr(IvaTasaDtl)) = 0 Then '01/26/23  the modificatin above, should actually be this one  ivatasadtl not lineiva.....
                                    DTL = DTL & "TASIPE  0" & vbNewLine & "MONIPE 0 " & vbNewLine & "FCTTASIPE   0.000000" & vbNewLine
                                    DTL = DTL & "IMPORTIPE " & VB6.Format(CSPrice * RS!csshipped.Value + CDbl(IEPSAmt), "##0.00") & vbNewLine 'Base to calculate Tasa 0%
                                    DTL = DTL & "TIPIPETR Tasa" & vbNewLine & "TASIEP 0" & vbNewLine
                                    IEPSnoIVA = IEPSnoIVA + CDbl(IEPSAmt)
                        ...
                        If DiscExists Then '02/09/18
                            DTL = DTL & "TDECON    " & DiscPer & vbNewLine
                            DTL = DTL & "MDECON    " & (CSPrice * RS!csshipped.Value) * (DiscPer / 100) & vbNewLine '02/16/18 ---> was using EaPrice here, so I changed to CsPrice
                        Else
                            DTL = DTL & "TDECON  " & "0" & vbNewLine & "MDECON  " & "0.00" & vbNewLine 'NO DESCUENTO EN DETALLE HARD CODE 0
                        End If
                    ...
                    Case Is = "CE" '----------- CS & EA
                        ...
                        PricePlusLiqTax = Val(RS!INVCSPRICE.Value) + (Val(RS!INVCSPRICE.Value) * (IEPSTasa / 100)) 'To calculate iva based on this
                        If LiqTax2 > 0 Then PricePlusLiqTax = Val(RS!INVCSPRICE.Value) + LiqTax2 '---12/26/13
                        ...
                        UnitPrice = VB6.Format(CSPrice, "##0.00")
                        Importe = VB6.Format(CDbl(UnitPrice) * RS!csshipped.Value, "##0.00###")
                        TotImp = TotImp + Val(Importe)
                        If Val(CStr(IEPSTasa)) > 0 Then IEPSAmt = VB6.Format((Val(CStr(CSPrice)) * RS!csshipped.Value) * (IEPSTasa / 100), "##0.00###") 'SAT IEPSAmt
                        If LiqTax2 > 0 Then IEPSAmt = VB6.Format(RS!csshipped.Value * LiqTax2, "##0.00") '12/23/13 ------------------------------
                        If (SeparateLiqTaxFlag = "Y" And Val(CStr(IEPSTasa)) > 0) Or (SeparateLiqTaxFlag = "Y" And LiqTax2 > 0) Then '---12/26/13
                            IEPSAmt_Prt = IEPSAmt 'Ieps printing
                            TotIEPS_Prt = CStr(Val(TotIEPS_Prt) + Val(IEPSAmt))
                        End If
                        If (SeparateLiqTaxFlag = "N" And Val(CStr(IEPSTasa)) > 0) Or (SeparateLiqTaxFlag = "N" And LiqTax2 > 0) Then 'Price should add IEPS on inv '
                            If Val(CStr(IEPSTasa)) > 0 Then UnitPrice = VB6.Format(RS!INVCSPRICE.Value + (RS!INVCSPRICE.Value * (IEPSTasa / 100)), "##0.00###")
                            If LiqTax2 > 0 Then UnitPrice = VB6.Format(RS!INVCSPRICE.Value + LiqTax2, "##0.00###") '12/23/2013 ------
                            Importe = VB6.Format(CDbl(UnitPrice) * RS!csshipped.Value, "##0.00###") '(calculation * qty does not match JETS)
                            CsImporte = CDbl(Importe) 'ONLY USED WHEN CS & EA FOR LIQUOR NO DESGLOSE CALCULATION... ON EACH
                            IEPSAmt_Prt = vbNullString
                        End If
                        TotImp_Prt = TotImp_Prt + Val(Importe)
                        If Val(CStr(IvaTasaDtl)) > 0 Then
                            IvaAmt = CStr(PricePlusLiqTax * RS!csshipped.Value * (IVATasa / 100))
                            IvaAmt = VB6.Format(IvaAmt, "0.00") '01/27/23 ADDED FORMAT, IF TOO MANY DECIMALS IT WON'T PROCESS....
                        ...
                        End If
                        CsImpIvaIeps = (Val(CStr(CSPrice)) * RS!csshipped.Value) + Val(IvaAmt) + Val(IEPSAmt)
                        '' CsImpIvaIeps = (Val(UnitPrice) * RS!CSSHIPPED.Value) + Val(IvaAmt)
                        Units = "CA"
                        UOMCatalog = "XBX" '06/21/18 JMX NOW WANTS TO CHANGE UNIT OF MEASURE TO BE 'XBX' IF CASES, OR 'H87' IF EACHES.
                        ...
                        If ((SeparateLiqTaxFlag = "N" And Val(CStr(IEPSTasa)) > 0) Or (SeparateLiqTaxFlag = "N" And LiqTax2 > 0)) And IEPS_NO_SeparateXml Then '-->>>04/09/19 ****SIR # 1794*** ------------->>>>>>>>>>>
                            ...
                            DTL = DTL & "PBRUDE  " & UnitPrice & vbNewLine
                            DTL = DTL & "VALUNI  " & UnitPrice & vbNewLine
                            DTL = DTL & "IMPBRU  " & VB6.Format(CDbl(UnitPrice) * RS!csshipped.Value, "##0.00###") & vbNewLine
                            DTL = DTL & "IMPORT  " & VB6.Format(CDbl(UnitPrice) * RS!csshipped.Value, "##0.00###") & vbNewLine
                        Else
                            If Val(CStr(IEPSTasa)) > 0 Then DTL = DTL & "TASIEP  " & IEPSTasa & vbNewLine & "MONIEP  " & IEPSAmt & vbNewLine 'VERSION 4. ADDED IF ONLY USE IF > 0
                            DTL = DTL & "PBRUDE  " & VB6.Format(CSPrice, "##0.00") & vbNewLine
                            DTL = DTL & "IMPBRU  " & VB6.Format(CSPrice * RS!csshipped.Value, "##0.00") & vbNewLine
                            DTL = DTL & "VALUNI  " & VB6.Format(CSPrice, "##0.00") & vbNewLine
                            DTL = DTL & "IMPORT  " & VB6.Format(CSPrice * RS!csshipped.Value, "##0.00") & vbNewLine
                        ...
                        If DiscExists Then
                            DTL = DTL & "TDECON    " & DiscPer & vbNewLine
                            DTL = DTL & "MDECON    " & (CSPrice * RS!csshipped.Value) * (DiscPer / 100) & vbNewLine
                        Else
                            DTL = DTL & "TDECON  " & "0" & vbNewLine & "MDECON  " & "0.00" & vbNewLine 'NO DESCUENTO EN DETALLE HARD CODE 0
                        End If
                        ...
                        If LiqTax2 > 0 Then '01/23/18---------SUGARY DRINKS!!!!!!!!!!------------------
                            ...
                            If DiscAmt = 0 And SeparateLiqTaxFlag = "Y" Then ' don't do this bellow for free items --07/14/23  ADDED SEPARATELIQTAXFLAG
                                ...
                                DTL = DTL & "TIPIEPTR    Cuota" & vbNewLine
                                'DTL = DTL & "IMPORTIEP    " & Format(Val(IEPSAmt) / IEPSperLiter, "0.######") & vbNewLine    '05/09/18 added /IEPSperLiter
                                DTL = DTL & "IMPORTIEP    " & VB6.Format(Val(IEPSAmt) / IEPSperLiter, "0.##") & vbNewLine '05/31/22 VERSION 4 USE ONLY TWO DIGITS, OTHERWISE ERRORS OUT
                                ...
                                If Val(CStr(IvaTasaDtl)) = 0 Then '01/26/23  the modificatin above, should actually be this one  ivatasadtl not lineiva.....
                                    DTL = DTL & "TASIPE 0 " & vbNewLine & "MONIPE 0" & vbNewLine & "FCTTASIPE 0.000000" & vbNewLine
                                    DTL = DTL & "TIPIPETR Tasa " & vbNewLine & "IMPORTIPE " & ((CSPrice * RS!csshipped.Value) + CDbl(IEPSAmt)) & vbNewLine
                                    IEPSnoIVA = IEPSnoIVA + CDbl(IEPSAmt)
                        ...
                        '************next line,EACH info *********************************************
                        d = d + 1
                        'PricePlusLiqTax = Val(RS!inveaprice.Value) + (Val(RS!inveaprice.Value) * (IEPStasa / 100)) '--12/26/13
                        PricePlusLiqTax = Val(RS!INVEAPRICE.Value) '--12/26/13 'Before taxes, if any tax, add bellow
                        If IEPSTasa > 0 Then PricePlusLiqTax = Val(RS!INVEAPRICE.Value) + (Val(RS!INVEAPRICE.Value) * (IEPSTasa / 100)) '12/26/13
                        If LiqTax2 > 0 Then PricePlusLiqTax = Val(RS!INVEAPRICE.Value) + (LiqTax2 / RS!eapercs.Value) '--12/26/13
                        '--2012 Oct 'If SlsTaxFlag Then TotAmtToBeTaxed = Val(TotAmtToBeTaxed) + Val(RS!lineprice.Value) + Val(RS!lineliqtax.Value) 'Amt for ea and cs combined we do it once

                        UnitPrice = VB6.Format(Eaprice, "##0.00")
                        Importe = VB6.Format(CDbl(UnitPrice) * RS!eashipped.Value, "##0.00###")
                        TotImp = TotImp + Val(Importe)
                        IEPSAmt = vbNullString
                        If Val(CStr(IEPSTasa)) > 0 Then IEPSAmt = VB6.Format((Val(CStr(Eaprice)) * RS!eashipped.Value) * (IEPSTasa / 100), "##0.00###") 'SAT IEPSAmt
                        If LiqTax2 > 0 Then IEPSAmt = VB6.Format(RS!eashipped.Value * (LiqTax2 / RS!eapercs.Value), "##0.00###") '12/23/13 ------------------------------
                        If SeparateLiqTaxFlag = "N" Then 'Price should add IEPS on inv '12/01/10 modify Ea and Cs prices on the file requested by WFactura
                            If IEPSTasa > 0 Then UnitPrice = VB6.Format(RS!INVEAPRICE.Value + (RS!INVEAPRICE.Value * (IEPSTasa / 100)), "##0.00###")
                            If LiqTax2 > 0 Then UnitPrice = VB6.Format(Eaprice + (LiqTax2 / RS!eapercs.Value), "##0.00###")
                            ...
                            If IEPSTasa = 0 And LiqTax2 = 0 Then 'CHANGED ONLY TO MATCH CALCULATION ON JETS IE 0243455 WHERE 34.16 * 12 = 409.86 INSTEAD OF 409.92
                                Importe = VB6.Format(CDbl(UnitPrice) * RS!eashipped.Value, "##0.00###") '01/26/11 (calculation * qty does not match JETS) Put it back on 2/17/11
                            Else
                                Importe = VB6.Format(Val(RS!lineprice.Value) + Val(RS!lineliqtax.Value) - CsImporte, "##0.00###")
                            End If
                        End If
                        If (SeparateLiqTaxFlag = "Y" And Val(CStr(IEPSTasa)) > 0) Or (SeparateLiqTaxFlag = "Y" And LiqTax2 > 0) Then
                            IEPSAmt_Prt = IEPSAmt 'Ieps printing
                        ...
                        If SeparateLiqTaxFlag = "Y" And LiqTax2 > 0 Then IEPSAmt = VB6.Format((LiqTax2 / RS!eapercs) * (RS!eashipped.Value), "##0.00")
                        If Val(CStr(IvaTasaDtl)) > 0 Then
                            IvaAmt = CStr(PricePlusLiqTax * RS!eashipped.Value * (IVATasa / 100))
                            IvaAmt = VB6.Format(IvaAmt, "0.00") '01/27/23 ADDED FORMAT, IF TOO MANY DECIMALS IT WON'T PROCESS
                            TotalTax_New = TotalTax_New + CDbl(IvaAmt) '----04/12/19 sir 1794, when ea and cs present one cent difference and it doesn't generate cfdi
                        End If
                        EaImpIvaIeps = (Val(CStr(Eaprice)) * RS!eashipped.Value) + Val(IvaAmt) + Val(IEPSAmt)
                        Units = "EA"
                        UOMCatalog = "H87" '06/21/18 JMX NOW WANTS TO CHANGE UNIT OF MEASURE TO BE 'XBX' IF CASES, OR 'H87' IF EACHES.
                        ...
                        Else
                            ...
                            Else
                                ...
                                Else
                                    ...
                                    DTL = DTL & "TASIPE  0" & vbNewLine & "MONIPE 0 " & vbNewLine & "FCTTASIPE   0.000000" & vbNewLine
                                    DTL = DTL & "IMPORTIPE " & VB6.Format(CDbl(UnitPrice) * RS!eashipped.Value, "##0.00") & vbNewLine
                                    DTL = DTL & "TIPIPETR Tasa" & vbNewLine & "TASIEP 0" & vbNewLine
                                End If
                            End If
                        End If
                        ...
                        If Eaprice = 0 Then 'FREE EA (WHEN CS AND EA PRESENT) !!!!!!!!!!
                            ...
                            'TotAmtBeforeTaxes = TotAmtBeforeTaxes + (0.01 * RS!eashipped.Value)
                            '---END VERSION 4.0  ---
                            DiscAmt = 0.01 * RS!eashipped.Value
                            IEPSTasa = 0 'In case free item has ieps do not report tasa because it won't have any ieps amount
                            LiqTax2 = 0 '01/25/24  In case free item, do not report any tasa, no ieps amt should be reported
                            TotDiscAmt = TotDiscAmt + DiscAmt
                        End If
                        ...
                        If ((SeparateLiqTaxFlag = "N" And Val(CStr(IEPSTasa)) > 0) Or (SeparateLiqTaxFlag = "N" And LiqTax2 > 0)) And IEPS_NO_SeparateXml Then '-->>>04/09/19 ****SIR # 1794*** ------------->>>>>>>>>>>
                            ...
                            DTL = DTL & "IMPORT  " & VB6.Format(CDbl(UnitPrice) * RS!eashipped.Value, "##0.00###") & vbNewLine
                        Else
                            If Val(CStr(IEPSTasa)) > 0 Then DTL = DTL & "TASIEP  " & IEPSTasa & vbNewLine & "MONIEP  " & IEPSAmt & vbNewLine 'VERSION 4 ADDED IF ? 0
                            DTL = DTL & "PBRUDE  " & VB6.Format(Eaprice, "##0.00") & vbNewLine
                            DTL = DTL & "VALUNI  " & VB6.Format(Eaprice, "##0.00") & vbNewLine
                            DTL = DTL & "IMPBRU  " & VB6.Format(Eaprice * RS!eashipped.Value, "##0.00") & vbNewLine
                            DTL = DTL & "IMPORT  " & VB6.Format(Eaprice * RS!eashipped.Value, "##0.00") & vbNewLine
                        ...
                        If LiqTax2 > 0 Then '01/23/18---------SUGARY DRINKS!!!!!!!!!!------------------
                            ...
                            If DiscAmt = 0 And SeparateLiqTaxFlag = "Y" Then ' don't do this bellow for free items 07/14/23 ADDED SEPARATELIQTAXFLAG
                                ...
                                DTL = DTL & "TIPIEPTR    Cuota" & vbNewLine
                                'DTL = DTL & "IMPORTIEP    " & Format(Val(IEPSAmt) / IEPSperLiter, "0.######") & vbNewLine
                                DTL = DTL & "IMPORTIEP    " & VB6.Format(Val(IEPSAmt) / IEPSperLiter, "0.##") & vbNewLine 'VERSION 4  USE ONLY TWO DECIMALS, OTHERWISE ERRORS OUT
                                DTL = DTL & "MONIEP " & Val(IEPSAmt) & vbNewLine 'ADDED VERSION 4.0  NOT SURE IF IT WAS ON PREVIOS VERSION
                ...
                End Select
                SQL = "select * from custom_doc where itemcode = '" & RS!itemcode.Value & "'"
                RSc = Db.CreateDynaset(SQL, &H4)
                DocID = vbNullString : DocDate = vbNullString : PortName = vbNullString
                If RSc.RecordCount > 0 Then
                    PortName = vbNullString & RSc!Port.Value
                    ...
                    If Not SorianaErr Then '07/23/23 Soriana some times has errors so we do not want  to insert in custom_doc_invoice table  because many entries could be inserted meantime the error is solved.
                        If Trim(DocID) <> vbNullString Then '07/21/23 if the docid < 15 in lenght, we dont report it because invoice will error out
                            ...
                            SQLInsert = "insert into custom_doc_invoice values ('" & InvNum & "','" & RS!itemcode.Value & "','" & DocID & "','" & VB6.Format(RSc!doc_date.Value, "dd-mmm-yyyy") & "','" & PortName & "')"
            ...
            DTLCPT = DTLCPT & "COM_CPT_MER_PESBRU " & VB6.Format(TotWeight, "##0.00") & vbNewLine ''PESO BRUTO HERE!!!!!!!!!!!!!!!!!!
            DTLCPT = DTLCPT & " COM_CPT_MER_PESONET " & VB6.Format(TotWeight, "##.00") & vbNewLine ' 'PESO NETO HERE!! using peso bruto as not all items have peso neto.
        ...
        Exit Sub
ErrHndlr:
        ErrMsgLog = New String("*", 50) & vbNewLine & VB6.Format(Now, "mm/dd/yy hh:mm") & " -> ERROR:  " & Err.Number & "-->" & Err.Description & "." & vbNewLine & "Error on GetDtl Routine.  Prog:  DigInv3." & vbNewLine & "Total Invoices:  " & TotInvs & vbNewLine & vbNewLine & "Last SQL ran : " & vbNewLine & SQL & vbNewLine & "Item being processed for line: " & d & " " & RS!itemcode.Value & vbNewLine & "Order being processed: " & InvNum & vbNewLine
        CkFileExists(DirToOutputError, ErrMsgLog, "ERR-Dig-Inv3_3.log")
        SendEmail(ErrMsgLog)
    ...
    Private Sub NoItemCdRoutine(ByRef InvNum As String, ByRef IEPSTasa As Double, ByRef IEPSAmt As String)
        ...
        ''Debug.Print SQL
        Rsd2 = Db.CreateDynaset(SQL, &H4)
        If Rsd2.RecordCount > 0 Then
            For j = 1 To Rsd2.RecordCount
                SeparateLiqTaxFlag = IepsSeparate(InvNum, Rsd2!linenum.Value) '11/01/16 City Fresko does not need to separate when printing, Comercial Mexica needs separation.
                Select Case (vbNullString & Rsd2!remarkid.Value)
                    Case Is = "MC", "AC" 'Nota de Cargo, Aditional charge
                        ...
                        TotImp_Prt = TotImp_Prt + Rsd2!lineprice.Value '07/11/14 if line items with lines with no itemcode, then we need to add, no overwrite TotImp_Prt is used in the total line
                        TotAmtToBeTaxed = Rsd2!lineprice.Value
                        UnitPrice = Rsd2!lineprice.Value '11/02/16  save unit price, this price will be used if no ieps found.
                        LineImpIvaIeps = Rsd2!lineprice.Value
                        ...
                        DTL = DTL & "VALUNI  " & Rsd2!lineprice.Value & vbNewLine & "IMPORT  " & Rsd2!lineprice.Value & vbNewLine
                        DTL = DTL & "PBRUDE  " & Rsd2!lineprice.Value & vbNewLine
                        DTL = DTL & "IMPBRU  " & Rsd2!lineprice.Value & vbNewLine '02/22/19 bug on nota de cargo was not writting IMPBRU
                        ...
                        If SeparateLiqTaxFlag = "Y" Then '11/01/16 -->> if separate, buld dtl, if not, build dtl later  on...
                            DTL = DTL & "PBRUDE_IEPS  " & Rsd2!lineprice.Value & vbNewLine
                            DTL = DTL & "IMPBRU_PRT  " & Rsd2!lineprice.Value & vbNewLine
                        End If '11/01/16
                    Case Is = "FC" 'Freight Charge is a complete line by itself. Others might combine up to 3 dtl lines in printing
                        If j > 1 Then DTL = DTL & "IMPIVAIEPS  " & LineImpIvaIeps & vbNewLine & "NUMLIN  " & TotLines & vbNewLine '--if more than one line of nota de cargo, finish the dtl line, otherwise, it will start wiht 2nd line without finishing 1st.
                        Create1stPartofDtl(Rsd2)
                        DTL = DTL & "PBRUDE  " & Rsd2!lineprice.Value & vbNewLine & "PBRUDE_IEPS  " & Rsd2!lineprice.Value & vbNewLine
                        DTL = DTL & "IMPBRU  " & Rsd2!lineprice.Value & vbNewLine & "IMPBRU_PRT  " & Rsd2!lineprice.Value & vbNewLine
                        DTL = DTL & "VALUNI  " & Rsd2!lineprice.Value & vbNewLine & "IMPORT  " & Rsd2!lineprice.Value & vbNewLine
                        If Val(CStr(IEPSTasa > 0)) Then DTL = DTL & "TASIEP  " & IEPSTasa & vbNewLine & "MONIEP  " & IEPSAmt & vbNewLine 'VERSION 4 ADDED IF ONLY IF >0
                        LineImpIvaIeps = Rsd2!lineprice.Value '05/22/15
                        TotImp_Prt = TotImp_Prt + Rsd2!lineprice.Value
                    Case Is = "ST"
                        ...
                        If Created2ndPart = False Then
                            ...
                            Created2ndPart = True
                        End If
                        'TotalTax = Val(Rsd2!lineprice.Value) '11/19/15 commented, replaced by bellow stmt
                        TotalTax = CStr(Val(TotalTax) + Val(Rsd2!lineprice.Value)) '--11/20/15  added val() for totaltax -- 11/19/15 bug found on printout of invoice:  39-0411482 420.00 printed instead of total 10,125.91
                        ...
                        LineImpIvaIeps = LineImpIvaIeps + Val(Rsd2!lineprice.Value) ' to correct bug found on invoice 39-0411482
                        '-------VERSION 4. ONLY USE THESE VALUES IF > 0
                        If Val(CStr(IVATasa)) > 0 Then
                            DTL = DTL & "MONIPE  " & Rsd2!lineprice.Value & vbNewLine 'This line of sls info will be added to dtl on line of NC, or if just iva, the main line was created above.
                    ...
                    Case Else ' L1, .... L5 (diff types of ieps) -- prog will work ONLY if just one type of  ieps in in dtl....
                        SQL = "select nvl(ieps_rate,0)ieps_rate from remark_code where remarkid = '" & Rsd2!remarkid.Value & "'"
                        Rsd3 = Db.CreateDynaset(SQL, &H4)
                        If Rsd3.RecordCount > 0 And (Rsd3!ieps_rate.Value) >= 0 Then 'printing ieps in separate line.  should be alone only if there is only one line.
                            IEPSTasa = Rsd3!ieps_rate.Value
                            IEPSAmt = Rsd2!lineliqtax.Value
                            ...
                            If Created2ndPart = False Then
                                ...
                                Created2ndPart = True
                            End If
                            If Val(CStr(IEPSTasa)) > 0 Then DTL = DTL & "TASIEP  " & IEPSTasa & vbNewLine & "MONIEP  " & IEPSAmt & vbNewLine 'VERSION 4. ADDED IF
                            DTL = DTL & "MONIEP_IEPS  " & IEPSAmt_Prt & vbNewLine
                            If Rsd2!lineprice.Value = 0 Then 'Line price is 0, use the amount in line total
                                TmpTotImp = Rsd2!linetotal.Value 'TmpTotImp is used in dtl for  import because value might be in lineprice or  line total, if i line price, rather read line price
                            Else
                                TmpTotImp = Rsd2!lineprice.Value
                            End If
                            If SeparateLiqTaxFlag = "Y" Then
                                LineImpIvaIeps = LineImpIvaIeps + CDbl(TmpTotImp) ' Rsd2!lineprice.Value
                                'TotImp_Prt = TotImp_Prt + Rsd2!lineprice.Value  '--->>> PENDING CK THIS ONE, I THINK THIS SHOULD BE COMMENTED.!!!!!!  --->>>>
                            Else 'DO NOT SEPARATE, ADD IEPS TO UNIT PRICE.
                                ...
                                LineImpIvaIeps = LineImpIvaIeps + CDbl(TmpTotImp) 'Rsd2!linetotal.Value  'Add ieps to the import amount to be printed
                            ...
                            FinishedDtl = True
                            '--------------------------
                            TotAmtToBeTaxed = TotAmtToBeTaxed + Rsd2!lineprice.Value
                        End If
                End Select
                Rsd2.movenext()
            Next j
...
ErrorHandler:  'write error file.
    ...
    End Sub
    '========================================
    Private Sub Create1stPartofDtl(ByRef Rsd2 As Object)
        '========================================
        DTL = DTL & "CANTID  " & "1" & vbNewLine & "CANTID_EA  " & "1" & vbNewLine & "DESCRI  " & Rsd2!linedesc.Value & vbNewLine
    ...
    Private Function IepsSeparate(ByRef InvNo As String, ByRef LineNo As String) As String
        ...
        RS = Db.CreateDynaset(SQL, &H0)
        If RS.RecordCount > 0 Then
            If Val("" & RS!SEPARATE.Value) = 1 Then
                IepsSeparate = "Y"
...
ErrorHandler:  'write error file.
    ...
    Private Function GetJustItemDesc(ByRef ItemCd As String) As String
        ...
        RS = Db.CreateDynaset(SQL, &H0)
        If RS.RecordCount > 0 Then
            GetJustItemDesc = vbNullString & RS!PURE_ITEMDESC.Value
        Else
            GetJustItemDesc = vbNullString
        End If
        Exit Function
ErrHndlr:
        ErrMsgLog = New String("*", 50) & vbNewLine & "Today: " & VB6.Format(Now, "mm/dd/yy hh:mm") & "---> Eror # " & Err.Number & "-->" & Err.Description & "." & vbNewLine & "Error on DigInv3 program, GetJustItemDesc Function" & vbNewLine & "SQL: " & SQL & vbNewLine
    ...
    Private Function GetMetodoPago(ByRef Cust As String) As String 'NOT BEING USED ANY MORE.  METODO DE PAGO IS HARD-CODED FOR ALL CUSTOMERS AS PPD
        ...
        If RS.RecordCount > 0 Then
            For i = 1 To RS.RecordCount
                If i > 1 Then
                    Metodo = Metodo & ", " & RS!LICENSENUM.Value
                Else
                    Metodo = vbNullString & RS!LICENSENUM.Value
                End If
                RS.movenext()
            Next i
        ...
        Exit Function
ErrHndlr:
        ErrMsgLog = New String("*", 50) & vbNewLine & "Today: " & VB6.Format(Now, "mm/dd/yy hh:mm") & "---> Eror # " & Err.Number & "-->" & Err.Description & "." & vbNewLine & "Error on DigInv3 program, GetMetodoPago sub" & vbNewLine & "SQL: " & SQL & vbNewLine
        ErrMsgLog = ErrMsgLog & "PROGRAM ENDED WITHOUT PROCESSING ANYTHING!!!!" & vbNewLine
    ...
    Sub CkFileExists(ByRef StrDir As String, ByRef StrData As String, ByRef StrFile As String)
        ...
        Exit Sub
ErrHndlr:
        ErrMsgLog = New String("*", 50) & vbNewLine & "Today: " & VB6.Format(Now, "mm/dd/yy hh:mm") & "---> Eror # " & Err.Number & "-->" & Err.Description & "." & vbNewLine & "Error on DigInv3 program, CkFileExists sub" & vbNewLine ' & "SQL: " & SQL & vbNewLine
        ErrMsgLog = ErrMsgLog & "PROGRAM ENDED WITHOUT PROCESSING ANYTHING!!!!" & vbNewLine '& Serie & Folio & vbNewLine
        Call CkFileExists(DirToOutputError, ErrMsgLog, "ERR-Dig-Inv3_3.log")
        SendEmail(ErrMsgLog)
    End Sub
    '==================================================================================================
    Private Function GetTermMX(ByRef RS As Object) As String
        '===================================================================================================
        If "" & RS!CreditCodeId = "A" Or "" & "" & RS!CreditCodeId.Value = "B" Then
            GetTermMX = "DUE: C.O.D"
            TermsDate = VB6.Format(RS!invdate.Value, "yyyy-mm-dd") '09/12/18 Added per Ma Elena's request because there was no date, it was defaulting 06-23-2018, don't know why.
        ElseIf "" & RS!codonliqinv.Value = "1" And "" & RS!sectioncode.Value = "5" Then
            GetTermMX = "DUE: C.O.D"
            TermsDate = VB6.Format(RS!invdate.Value, "yyyy-mm-dd") '09/12/18 Added per Ma Elena's request because there was no date, it was defaulting 06-23-2018, don't know why.
        Else
            GetTermMX = Term2MX(RS)
        End If
    End Function
    '================================================
    Private Function Term2MX(ByRef RS As Object) As String
        '=================================================
        If RS!DISCPCNT.Value = 0 Then
            If RS!netdays.Value = 0 And RS!ardays.Value = 0 Then
                Term2MX = "DUE:" & VB6.Format(RS!invdate.Value + 30, "DD-MM-yy")
                TermsDate = VB6.Format(RS!invdate.Value + 30, "yyyy-mm-dd") '11/21/17
            Else
                If RS!ardays.Value > RS!netdays.Value And RS!netdays.Value <> 0 Then
                    Term2MX = "NET " & RS!netdays.Value & " DAYS"
                    TermsDate = VB6.Format(RS!invdate.Value + RS!netdays.Value, "yyyy-mm-dd") '11/21/17
                ElseIf RS!ardays.Value = 0 And RS!netdays.Value > 0 Then  'ARDays = 0, NetDays > 0
                    Term2MX = "NET " & RS!netdays.Value & " DAYS"
                    TermsDate = VB6.Format(RS!invdate.Value + RS!netdays.Value, "yyyy-mm-dd") '11/21/17
                Else
                    If RS!ardays.Value < 30 Then
                        Term2MX = "DUE:" & VB6.Format(RS!invdate.Value + RS!ardays.Value, "DD-MM-YY")
                        TermsDate = VB6.Format(RS!invdate.Value + RS!ardays.Value, "yyyy-mm-dd") '11/21/17
                    Else
                        Term2MX = "DUE:" & VB6.Format(RS!invdate.Value + 30, "DD-MM-YY")
                        TermsDate = VB6.Format(RS!invdate.Value + 30, "yyyy-mm-dd") '11/21/17
                    End If
            ...
            End If
        Else
            If RS!netdays.Value <> 0 Then
                Term2MX = VB6.Format(RS!DiscPct.Value, "#0") & "% " & VB6.Format(RS!DISCDAYS.Value, "#0") & "/NET " & VB6.Format(RS!netdays.Value, "#0")
                TermsDate = VB6.Format(RS!invdate.Value + RS!netdays.Value, "yyyy-mm-dd") '11/21/17
            Else
                Term2MX = VB6.Format(RS!DiscPct.Value, "#0") & "% " & VB6.Format(RS!DISCDAYS.Value, "#0")
                TermsDate = VB6.Format(RS!invdate.Value + 30, "yyyy-mm-dd") '11/21/17
            End If
        End If
    End Function
    ...
    Private Sub UpdateInvHdr(ByRef InvNum As String, ByRef Serie As String, ByRef Folio As String)
        ...
        If RS.RecordCount > 0 Then
            RS.edit()
            RS!refinvnum.Value = Serie & Folio
            RS.Update()
        End If
        Exit Sub
ErrHndlr:
        ErrMsgLog = New String("*", 50) & vbNewLine & "Today: " & VB6.Format(Now, "mm/dd/yy hh:mm") & "---> Eror # " & Err.Number & "-->" & Err.Description & "." & vbNewLine & "Error on DigInv3 program, UpdateInvHdr sub" & vbNewLine & "SQL: " & SQL & vbNewLine
        ErrMsgLog = ErrMsgLog & "PROGRAM ENDED WITHOUT PROCESSING ANYTHING!!!!" & vbNewLine & Serie & Folio & vbNewLine
        Call CkFileExists(DirToOutputError, ErrMsgLog, "ERR-Dig-Inv3_3.log")
        SendEmail(ErrMsgLog)
        End
    ...
    Private Sub SendEmail(ByRef MSG As String)
        ...
        With ObjMessage
            ...
            Body = Subject & vbNewLine & "Today: " & VB6.Format(Now, "mm/dd/yyyy hh:mm") & vbNewLine & "Check error folder on JMX: " & DirToOutputError & vbNewLine & vbNewLine & MSG
            .textBody = Body
        ...
        Exit Sub
ErrHndlr:
        ErrMsgLog = "Today: " & VB6.Format(Now, "mm/dd/yy hh:mm") & "---> Eror # " & Err.Number & "-->" & Err.Description & "." & vbNewLine & "Error on DigInv2 program, SendEmail sub" & vbNewLine
